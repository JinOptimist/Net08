...
namespace WebMazeMvc.EfStuff
{
    ...
    {
        public const string NinaName = "Nina";
        ...
        private static void InitCats(IServiceProvider service)
        {
            var catRepository = service.GetService<CatRepository>();
            var userRepository = service.GetService<UserRepository>();
            var nina = userRepository.Get(NinaName);
            var catDefaults = new List<Cat>() {
                new Cat()
                {
                    Name= "Kity",
            ...
            foreach (var cat in catDefaults)
            {
                if (!catRepository.Exist(cat.Name))
                {
                    cat.Creater = nina;
